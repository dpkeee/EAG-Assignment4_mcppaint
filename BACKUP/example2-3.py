...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        """Draw a rectangle in Paint from (start_x, start_y) to (end_x, end_y)"""
    

    # Step 2: Get the Paint window and maximize it
    paint_window = win32gui.FindWindow(None, "Untitled - Paint")  # Adjust title if needed
    if paint_window:
        win32gui.ShowWindow(paint_window, win32con.SW_MAXIMIZE)  # Maximize the window
        win32gui.SetForegroundWindow(paint_window)  # Bring the window to the front
    else:
    ...
    # Step 4: Draw a rectangle on canvas
    # Move to the starting point of the rectangle
    pyautogui.moveTo(start_x, start_y)  # Starting point
    ...
    pyautogui.moveTo(end_x, end_y)  # End point
    pyautogui.mouseUp()   # pyautogui.moveTo(842, 146) 
        # # Example position of the rectangle tool
        # pyautogui.click()
        # time.sleep(2)

        # # Step 3: Draw a rectangle on canvas
        # # Move to the starting point of the rectangle
        # pyautogui.moveTo(x1, y1)  # Use provided coordinates
        # pyautogui.mouseDown()  # Press down the mouse button to start drawing
        # time.sleep(2)  # Short delay to ensure the mouse is down
...
async def add_text_in_paint(text: str) -> dict:
    ...
    try:
        ...
        pyautogui.typewrite('final answer is ' + text)
        return {
            "content": [
                TextContent(
                    type="text",
